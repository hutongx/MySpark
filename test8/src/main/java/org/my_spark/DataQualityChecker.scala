...
class DataQualityChecker(spark: SparkSession) {
    ...
    private def performRangeCheck(df: DataFrame, tableName: String): List[String] = {
        ...
        // 修复方案 1：使用显式参数名
        val numericColumns = df.dtypes.filter(t =>
            t._2.contains("Int") || t._2.contains("Double") || t._2.contains("Float")
        ).map(_._1)
    ...
    private def performFormatCheck(df: DataFrame, tableName: String): List[String] = {
        ...
        val dateColumns = df.dtypes.filter(t =>
            t._2.contains("Date") || t._2.contains("Timestamp")
        ).map(_._1)
